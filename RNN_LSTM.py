...
EPCOHS = 5            #  an arbitrary cutoff, generally defined as "one pass over the entire dataset", used to separate training into distinct phases, which is useful for logging and periodic evaluation.
BATCH_SIZE = 1000        # a set of N samples. The samples in a batch are processed` independently, in parallel. If training, a batch results in only one update to the model.
INPUT_DIM = 20           # a vocabulary of 4 words in case of fnn sequence (ATCG)
OUTPUT_DIM = 40          # Embedding output
RNN_HIDDEN_DIM = 40
DROPOUT_RATIO = 0.2 # proportion of neurones not used for training
MAXLEN = 1000 # cuts text after number of these characters in pad_sequences
checkpoint_dir ='checkpoints'
os.path.exists(checkpoint_dir)

input_file = 'MSDBP_7129_7060_14189_train_RNN_LSTM_X_removed.csv'
